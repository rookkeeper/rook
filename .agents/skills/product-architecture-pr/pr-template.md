## What changed

[2–5 bullets. Keep this very skimmable. What changed at a product/architecture level?]

## Why it matters

[Required. 1 short paragraph or 2–4 bullets. What gets better for the user, system, or team?]

## Notes for reviewers

[Optional. Anything worth watching: tradeoffs, rollout notes, follow-ups, known limitations, or one important implementation detail.]

## Alternatives / tradeoffs

[Optional. Only include this when there was a real choice worth documenting. Keep it to a few bullets, not a long comparison.]

## Product specification alignment

**Classification:** [Implements | Extends | New concept | Modifies | Supersedes/removes]

**Related PRODUCT/ docs:**
- [`path/to/doc.md`](path/to/doc.md) — [how this PR relates]

**Documentation updates in this PR:**
- [ ] [`path/to/doc.md`](path/to/doc.md) — [what changed]
- _or:_ No PRODUCT/ updates — [reason, e.g. pure refactor with no spec impact]

**Notes:** [Gaps filled, open questions resolved, or intentional deferrals]

## Architecture alignment

**Classification:** [Implements | Extends | New concept | Modifies | Supersedes/removes]

**Related docs / patterns:**
- [doc or pattern name] — [relationship]

**Documentation updates in this PR:**
- [ ] [path or pattern doc] — [what changed]
- _or:_ No architecture doc updates — [reason]

**Notes:** [Boundary changes, new components, deprecated patterns]

## New tests

_If tests changed, summarize the new behavior covered at a high level. Keep this to a few bullets; do not list every test case._

- [Behavior or boundary covered]
- [Regression or integration behavior covered]

## Technical touchpoints

_Optional. Abstract only — major types, services, APIs, events, schemas._

- **Components:** [e.g. EnvironmentManager, skill discovery]
- **APIs / protocols:** [e.g. interact_with_environment, WebSocket payload]
- **Data / events:** [e.g. EnvironmentStateDelta schema]

## How to check it

- [ ] [How reviewer verifies the main behavior]
- [ ] [Regression or edge case checked]
- [ ] [Docs render / links valid]

