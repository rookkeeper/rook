...
The server is the stable broker. Clients are thin native surfaces over the same REST + ACP contract. Environment-repository storage, capability hashing, materialization, and authoring details are documented in [server.md](./server.md) and [database.md](./database.md).
