...
The launcher uses the main checkout as the production-like local profile. Running the same command from a Git worktree starts an isolated development profile with its own port, SQLite database, `~/.rook-<worktree-slug>` state directory, logs, and Mac app identity. The slug includes a short hash of the canonical worktree path so same-named worktrees remain distinct. Development profiles inherit the configured runtime catalog from production by default, while session/server state remains isolated. Use `ROOK_RUN_MODE` or `ROOK_PRODUCTION_ROOT` for explicit profile selection.
