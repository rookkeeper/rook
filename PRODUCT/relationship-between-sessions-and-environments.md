...
If a runtime is replaced for any other reason, the server privately performs the same persisted-session load before forwarding the next prompt. On the first request after a server restart, persisted session-environment memberships are rehydrated from repositories before workspace materialization and runtime recovery; unavailable environments remain visible as entered unavailable entries without deleting their durable membership, and non-directory environments retain their deterministic personal authoring projection. The replay is discarded before the replacement is attached to visible subscribers, so clients do not repaint the existing conversation.
