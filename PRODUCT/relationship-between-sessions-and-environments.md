...
If a runtime is replaced for any other reason, the server privately performs the same persisted-session load before forwarding the next prompt. The replay is discarded before the replacement is attached to visible subscribers, so clients do not repaint the existing conversation.
