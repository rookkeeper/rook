...
Keep the existing SQLite `session_environments` membership and bundle-hash approval model. During lazy session restoration, rehydrate persisted environment IDs from the repository into the fresh in-memory environment manager before materializing the session workspace and recovering the ACP session. If an environment is not currently observed, leave its durable membership intact, surface it as a recent entered entry, and resolve all repository-backed bundles plus the deterministic personal authoring projection for non-directory environments. Do not persist the transient availability cache or add client-facing API contracts.
