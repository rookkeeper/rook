...
- [ ] Choose and document the logical transcript persistence representation while preserving the existing REST contract where practical.
- [ ] Add per-session transcript aggregation/serialization in the server capture path.
- [ ] Merge contiguous user, assistant, and thought chunks into logical records with correct boundaries.
- [ ] Merge tool-call updates by `toolCallId`, handling snapshots, deltas, final statuses, and output ordering correctly.
- [ ] Persist an in-progress logical record as a current snapshot, with finalization at run/message boundaries.
- [ ] Coalesce or migrate existing chunk-level transcript rows and verify old sessions hydrate correctly.
- [ ] Add server tests for aggregation, ordering, tool updates, active-turn snapshots, and legacy transcript reads.
- [ ] Add or extend RookKit tests for transcript hydration and block counts/content.
- [ ] Run the appropriate server, RookKit, and Apple build/test checks and confirm they pass.
...
- [ ] Existing sessions with chunk-level rows hydrate coherently.
