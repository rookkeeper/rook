...
- **Sessions** - session history is divided into durable Pinned and recency-sorted Recent sections; pinned sessions are excluded from Recent. Resume any session by clicking it, drag a row into Pinned to pin it, or use the row action menu for pin/unpin, rename, and delete. Discovery is REST (`GET /api/sessions`), management uses `PATCH/DELETE /api/sessions/:id`, and opening/resuming a session records a REST touch so it moves to the top even without a new prompt. New chats start on one unbound WebSocket that becomes session-bound after `session/new`; thereafter live interaction is one session WebSocket per session.
