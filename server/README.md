...
The server's user-local repository, environment-authoring bindings, and application database honor `ROOK_HOME`. Worktree slugs include a short path hash, so same-named worktrees receive separate homes. When the profile home does not exist, the launcher seeds it by copying `~/.rook`, then removes the copied application database so the development profile starts without inherited production session history; subsequent configuration changes are isolated. <!-- THIS IS FOR BACKWARDS COMPATIBILITY: the launcher still migrates an existing production application database from the old repo-local `.var/rook/rook.sqlite` path into `ROOK_HOME/rook.sqlite` so users keep their session history while the default storage location changes. --> Existing production databases at the legacy repo-local `.var/rook/rook.sqlite` path are migrated into `ROOK_HOME/rook.sqlite` on first launcher startup unless the launcher is given an explicit `RUN_ROOK_DATABASE_PATH` override. `ROOK_AGENT_RUNTIMES_PATH` remains available as an explicit override. When starting the server through `run-rook.sh`, use `RUN_ROOK_HOME` / `RUN_ROOK_DATABASE_PATH` rather than ambient `ROOK_HOME` / `ROOK_DATABASE_PATH` to override the selected profile paths. The environment-repository API and bundle layout are unchanged.
