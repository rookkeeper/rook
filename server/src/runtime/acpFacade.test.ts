...
describe("ACP facade integration", { timeout: 30000 }, () => {
  ...
  it("allows a prompt to remain pending without runtime updates", async () => {
    const ws = await connect();
    await request(ws, 1, "initialize", { protocolVersion: 1, clientCapabilities: {}, clientInfo: { name: "quiet-prompt-test" } });
    const created = await request(ws, 2, "session/new", {
      cwd: "/tmp",
      mcpServers: [],
      _meta: { runtimeId: "MockAcpAgent", title: "quiet-prompt-test" },
    });
    const result = await request(ws, 3, "session/prompt", { sessionId: created.sessionId, prompt: [{ type: "text", text: "delayed response" }] });
    expect(result.stopReason).toBe("end_turn");
    await request(ws, 4, "session/close", { sessionId: created.sessionId });
    ws.close();
  });
