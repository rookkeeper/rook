...
export const PERSONAL_PUBLISHER = "personal";
const LEGACY_PERSONAL_PUBLISHER = "default";
...
export class SQLiteEnvironmentRepository extends EnvironmentRepository {
  ...
  async getBundles(environmentId: string): Promise<EnvironmentBundleResult> {
    ...
    const rows = this.db.prepare(`
      ...
      WHERE b.environment_id = ? AND b.deleted_at IS NULL AND ${this.publisherWhere()}
  ...
  async listEnvironments(): Promise<EnvironmentRecord[]> {
    return this.db.prepare(`
      SELECT e.environment_id, e.display_name, e.description, e.metadata_json
      FROM environments e
      WHERE EXISTS (
        SELECT 1 FROM bundles b
        WHERE b.environment_id = e.environment_id AND b.deleted_at IS NULL AND ${this.publisherWhere()}
      )
  ...
  saveResult(result: EnvironmentBundleResult): void {
    ...
    try {
      ...
      this.db.prepare(`DELETE FROM bundles WHERE environment_id = ? AND ${this.publisherWhere("bundles")}`).run(environment.id);
  ...
  async replaceCapabilityFiles(environmentId: string, bundleId: string, type: CapabilityType, capabilityName: string, files: Record<string, string>): Promise<boolean> {
    ...
    this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?, NULL)
      ON CONFLICT(bundle_id, capability_id) DO UPDATE SET environment_id = excluded.environment_id, publisher = excluded.publisher, deleted_at = NULL
    `).run(bundleId, environmentId, capabilityId, this.publisherForWrites());
    return true;
  }
  ...
  async deleteCapability(environmentId: string, bundleId: string, type: CapabilityType, capabilityName: string): Promise<boolean> {
    ...
    this.db.prepare(`UPDATE bundles SET deleted_at = ? WHERE bundle_id = ? AND environment_id = ? AND capability_id = ? AND ${this.publisherWhere("bundles")}`)
  ...
  async restoreCapability(environmentId: string, bundleId: string, type: CapabilityType, capabilityName: string): Promise<boolean> {
    ...
    const membership = this.findMembership(environmentId, bundleId, type, capabilityName);
    if (!membership) return false;
    this.db.prepare(`UPDATE bundles SET deleted_at = NULL WHERE bundle_id = ? AND environment_id = ? AND capability_id = ? AND ${this.publisherWhere("bundles")}`)
  ...
  protected writeBundle(bundle: EnvironmentBundle, bundleId: string, publisher = this.publisherForWrites()): void {
    this.db.prepare(`DELETE FROM bundles WHERE bundle_id = ? AND environment_id = ? AND ${this.publisherWhere("bundles")}`)
  ...
  protected upsertEnvironment(environment: EnvironmentRecord): void {
    const existing = this.db.prepare("SELECT display_name, description, metadata_json FROM environments WHERE environment_id = ?")
      .get(environment.id) as { display_name?: string; description?: string; metadata_json?: string } | undefined;
    const metadata = { ...parseMetadata(existing?.metadata_json), ...(environment.metadata ?? {}) };
    const displayName = existing?.display_name ?? environment.displayName;
    const description = existing?.description ?? environment.description;
    this.db.prepare(`
      INSERT INTO environments (environment_id, display_name, description, metadata_json)
      VALUES (?, ?, ?, ?)
      ON CONFLICT(environment_id) DO UPDATE SET
        metadata_json = excluded.metadata_json
    `).run(environment.id, displayName, description, JSON.stringify(metadata));
  }
  ...
  private findMembership(environmentId: string, bundleId: string, type: CapabilityType, name: string): { capabilityId: string; deletedAt: string | null } | undefined {
    const row = this.db.prepare(`
      ...
      WHERE b.environment_id = ? AND b.bundle_id = ? AND c.type = ? AND c.name = ? AND ${this.publisherWhere("b")}
      LIMIT 1
  ...
  /** SQL predicate for the logical source projected from bundle publisher metadata. */
  protected publisherWhere(alias = "b"): string {
    if (this.repositoryId === "personal") return `(${alias}.publisher = '${PERSONAL_PUBLISHER}' OR ${alias}.publisher = '${LEGACY_PERSONAL_PUBLISHER}')`;
    if (this.repositoryId === "web") return `(${alias}.publisher <> '${PERSONAL_PUBLISHER}' AND ${alias}.publisher <> '${LEGACY_PERSONAL_PUBLISHER}')`;
    return "1 = 1";
  }

  protected publisherForWrites(): string {
    return this.repositoryId === "personal" ? PERSONAL_PUBLISHER : LEGACY_PERSONAL_PUBLISHER;
  }
}
