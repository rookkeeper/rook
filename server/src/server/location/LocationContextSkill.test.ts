...
function candidate(over: Partial<EnvironmentCandidate>): EnvironmentCandidate {
  return {
    environmentId: "location:x.com/tn-1-main",
    displayName: "X",
    confidence: 0.9,
...
const current = candidate({
  environmentId: "location:cicis.com/tn-37211-5705-nolensville-pike",
  displayName: "Cicis",
  operator: "Cicis",
...
const neighbor = candidate({ environmentId: "location:gamestop.com/tn-37211-5705", displayName: "GameStop", operator: "GameStop", address: "5705 Nolensville Pike Ste 2" });
...
describe("location context skill", () => {
  it("puts the current env id in frontmatter and lists current + nearby metadata", () => {
    ...
    expect(md).toContain("environment: location:cicis.com/tn-37211-5705-nolensville-pike");
